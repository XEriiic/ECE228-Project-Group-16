...
def initialize(X, num_clusters):
    ...
    indices_row = np.tile(indices_row, X.shape[0]) # repeat once per batch item
    # print(indices_row.size)

    # indices_row = np.array([100, 300, 500, 700]*8)
    indices_col = np.arange(X.shape[0]).repeat(num_clusters)
    # print('indices_row', indices_row.size, 'indices_col', indices_col.size)

    initial_state = X[indices_col, indices_row, :].reshape(X.shape[0], num_clusters, -1)
    # print('combine', X[indices_col, indices_row, :].size(), 'initial_state after reshape', initial_state.size())
...
def kmeans(
    ...
    while True:
        # print('asasd', X.size(), initial_state.size())
